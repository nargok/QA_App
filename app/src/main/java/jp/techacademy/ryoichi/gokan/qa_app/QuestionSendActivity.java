...
public class QuestionSendActivity extends AppCompatActivity implements View.OnClickListener, DatabaseReference.CompletionListener {
    ...
    @Override // Intent連携から戻ってきたときに画像を取得し、ImageViewに設定する
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == CHOOSER_REQUEST_CODE) {
            ...
            Uri uri;
            if (data == null) { // カメラで撮影した場合
                uri = mPictureUri;
            } else if (data.getData() == null) { // カメラで撮影した場合
                uri = mPictureUri;
            } else { // ギャラリーから画像を取得した場合
                uri = data.getData();
            }
    ...
    public void onClick(View v) {
        if (v == mImageView) {
            ...
            // 第2引数にはCompleteListenerを設定する　画像を保存するのに時間がかかるため、
            // 完了をキャッチできるようにする
    ...
    @Override // 許可ダイアログでユーザが選択した結果を受け取る
    ...
    // ギャラリーから画像を選択するIntentとカメラで撮影するIntentを作成する
    // さらにどちらかを洗濯するIntentを作成してダイアログを表示する
    private void showChooser() {
        ...
        // ギャラリー選択のIntentを与えてcreateChooserメソッドを呼ぶ　1つめのintentとタイトルを渡す
        Intent chooserIntent = Intent.createChooser(galleryIntent, "画像を取得");

        // EXTRA_INITIAL_INTENTSにカメラ撮影のIntentを追加
        // 2つ目のIntentを第2引数に渡すことで2つのIntentを選択するダイアログを表示する
    ...
    public void onComplete(DatabaseError databaseError, DatabaseReference databaseReference) {
        ...
        if (databaseError == null) {
            finish(); // Firebaseへの保存が完了したらfinishメソッドでActivityを閉じる
        } else {
